...
2. Service **Settings** → **Source**:
   - **Root Directory:** `api-laravel`  ← **အရေးကြီးဆုံး**
   - **Builder:** `Dockerfile`
3. **Save** → **Redeploy**

> **Build failed in 1 second?**  
> Root Directory က `api-laravel` မဟုတ်ဘဲ repo root မှာ Flutter (`pubspec.yaml`) ကို build လုပ်နေလို့ပါ။  
> Settings မှာ Root Directory = `api-laravel` ပြင်ပြီး Redeploy လုပ်ပါ။
...
`MYSQL_ATTR_SSL_CA` **မထည့်ပါနဲ့** — Railway internal MySQL မှာ မလို။ ထည့်ထားရင် **ဖျက်ပါ**။

### C) လိုအပ်တဲ့ variables စာရင်း (အားလုံး ရှိရမယ်)

| Variable | ဥပမာ / မှတ်ချက် |
|----------|----------------|
| `APP_KEY` | `base64:...` ← **မဖြစ်မနေ** |
| `ADMIN_API_KEY` | random string (API only, not shown in app) |
| `ADMIN_PASSWORD` | your private admin login password |
| `ADMIN_PANEL_PATH` | secret path e.g. `lso-mgmt-moegyi-7k3p` (not `/admin`) |
| `ADMIN_ALLOWED_IPS` | optional: your public IP only |
| `APP_ENV` | `production` |
| `APP_DEBUG` | `false` |
| `APP_URL` | `https://lotaya-shwe-oh-production-d73b.up.railway.app` |
...
> Screenshot မှာ `APP_KEY` နဲ့ `DB_HOST` မပါရင် healthcheck **fail** ဖြစ်ပါမယ်။

### D) MySQL ချိတ်နည်း (လွယ်ကူသော နည်း)

1. **MySQL** service box → **Connect** (သို့မဟုတ် Laravel service Variables မှာ **Add Variable Reference**)
2. **MySQL** ရွေးပြီး `MYSQLHOST`, `MYSQLPORT`, ... auto ထည့်ပါ
3. Laravel ဘက်မှာ name ကို `DB_HOST`, `DB_PORT` အဖြစ် rename လုပ်ပါ

### E) Healthcheck failed?

Deploy အောင်ပြီး **Healthcheck failure** ဆိုရင်:
- `APP_KEY` + `DB_*` အားလုံး ရှိမရှိ စစ်
- `MYSQL_ATTR_SSL_CA` ဖျက်
- GitHub push နောက်ဆုံး code (healthcheck fix) redeploy
...
3. URL ရမယ် — ဥပမာ:  
   `https://lotaya-shwe-oh-production-d73b.up.railway.app`
...
| Private Admin | `https://YOUR-URL.up.railway.app/YOUR-ADMIN-PANEL-PATH` (bookmark only, not public) |
